...
class UserForm(FlaskForm):
    username = StringField('User name', validators=[Required(), Length(3, 24), Regexp('[a-zA-Z0-9]+$')])
    email = StringField('Email', validators=[Required(), Email()])
    password = PasswordField('Password', validators=[Required(), Length(6, 24)])
    #role = SelectField('Role', choices=[('10', 'user'), ('30', 'admin'), ('20', 'staff')], coerce=int)
    #job = StringField('Job', validators=[Required()])
    submit = SubmitField('Submit')

    def validate_name(self, field):
        if User.query.get('self.name.data'):
            raise ValidationError('The username was already used!')
